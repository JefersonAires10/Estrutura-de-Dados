...
public:
    ...
    ~Vector() {
        delete [] m_list;
        //cout << "a lista foi deletada" << endl;
    }
    ...
    int& at(int k) {
        if (k < 0 || k >= m_size) {
            std::runtime_error("Indice invalido");
        }
        return m_list[k];
    }
    const int& at(int k) const {
       // cout << "at const chamado" << endl;
        if (k < 0 || k >= m_size) {
            std::runtime_error("Indice invalido");
        }
        return m_list[k];
    ...
    void reserve(int n) {
        if (n > m_capacity) { // aumenta a capacidade do vetor
        int * aux = new int[n];
    ...
    void push_back(const int& value) {
        if (m_size == m_capacity) { // lista cheia 
